...
contract PreSaleContract is Ownable, ReentrancyGuard {
    ...
    IERC20 private constant TALContract =
        IERC20(0xcD883A18f8d33cf823D13cf2c6787c913d09e640);
    IERC20 private constant USDTContract =
    ...
    function getRoundIsFinished(uint256 round) external view returns (bool) {
        ...
        if (roundTimestamp <= block.timestamp) return true;
        ...
        checkRoundTalSupply(TALAmount, round)
    {
        ...
        uint256 totalBNBAmount = (USDPerTALPrice * TALAmount) /
            latestUSDPerBNBPrice;
        uint256 BNBPerTALPrice = (USDPerTALPrice * 10 ** decimals) /
            latestUSDPerBNBPrice;
        require(
            msg.value >= (totalBNBAmount - BNBPerTALPrice) &&
                msg.value <= (totalBNBAmount + BNBPerTALPrice),
            "The BNB amount is not equal"
        );
    ...
    function buyTALTokenByUSDT(
        ...
        USDTContract.transferFrom(msg.sender, owner(), totalUSDTAmount);

        TALContract.transfer(msg.sender, TALAmount);
        talSupplies[round - 1] -= TALAmount;

        emit BuyTALTokenByUSDT(msg.sender, TALAmount, totalUSDTAmount);
    }
    ...
    event BuyTALTokenByCash(address indexed buyer, uint256 talAmount);
}
